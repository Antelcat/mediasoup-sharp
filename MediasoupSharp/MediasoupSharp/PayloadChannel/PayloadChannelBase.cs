...
namespace MediasoupSharp.PayloadChannel
{
    public abstract class PayloadChannelBase : IPayloadChannel
    {
        ...
        {
            var id = InterlockedExtension.Increment(ref _nextId);
        ...
        {
            _logger.LogDebug($"RequestAsync() | Worker[{_workerId}] Method:{
            ...
            await using (await _closeLock.ReadLockAsync())
            {
                ...
                tcs.WithTimeout(TimeSpan.FromSeconds(15 + (0.1 * _sents.Count)),
