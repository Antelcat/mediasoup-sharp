...
namespace MediasoupSharp.Router
{
    public sealed class Router : EventEmitter, IEquatable<Router>
    {
        ...
        /// Transports map.
        /// </summary>
        private readonly Dictionary<string, Transport.Transport> _transports = new();
        private readonly AsyncReaderWriterLock _transportsLock = new();

        /// <summary>
        /// Producers map.
        /// </summary>
        private readonly Dictionary<string, Producer.Producer> _producers = new();
        private readonly AsyncReaderWriterLock _producersLock = new();

        /// <summary>
        /// RtpObservers map.
        /// </summary>
        private readonly Dictionary<string, RtpObserver.RtpObserver> _rtpObservers = new();
        private readonly AsyncReaderWriterLock _rtpObserversLock = new();

        /// <summary>
        /// DataProducers map.
        /// </summary>
        private readonly Dictionary<string, DataProducer.DataProducer> _dataProducers = new();
        private readonly AsyncReaderWriterLock _dataProducersLock = new();

        /// <summary>
        /// Router to PipeTransport map.
        /// </summary>
        private readonly Dictionary<Router, PipeTransport.PipeTransport[]> _mapRouterPipeTransports = new();
        ...
        private async Task CloseInternalAsync()
        {
            ...
            await using (await _rtpObserversLock.WriteLockAsync())
            {
            ...
            await using (await _dataProducersLock.WriteLockAsync())
            {
                // Clear the DataProducers map.
                _dataProducers.Clear();
            }

            await using (await _mapRouterPipeTransportsLock.WriteLockAsync())
            {
        ...
        public async Task<WebRtcTransport.WebRtcTransport> CreateWebRtcTransportAsync(WebRtcTransportOptions webRtcTransportOptions)
        {
            ...
            using (await _closeLock.ReadLockAsync())
            {
                ...
                var transport = new WebRtcTransport.WebRtcTransport(_loggerFactory,
        ...
        public async Task<PlainTransport.PlainTransport> CreatePlainTransportAsync(PlainTransportOptions plainTransportOptions)
        {
            ...
            using (await _closeLock.ReadLockAsync())
            {
                ...
                var transport = new PlainTransport.PlainTransport(_loggerFactory,
                                    new TransportInternal(_internal.RouterId, reqData.TransportId),
                                    responseData, // 直接使用返回值
                                    _channel,
                                    _payloadChannel,
        ...
        public async Task<PipeTransport.PipeTransport> CreatePipeTransportAsync(PipeTransportOptions pipeTransportOptions)
        {
            ...
            using (await _closeLock.ReadLockAsync())
            {
                ...
                var transport = new PipeTransport.PipeTransport(_loggerFactory,
                                    new TransportInternal(_internal.RouterId, reqData.TransportId),
                                    responseData, // 直接使用返回值
                                    _channel,
                                    _payloadChannel,
        ...
        public async Task<DirectTransport.DirectTransport> CreateDirectTransportAsync(DirectTransportOptions directTransportOptions)
        {
            ...
            using (await _closeLock.ReadLockAsync())
            {
                ...
                var transport = new DirectTransport.DirectTransport(_loggerFactory,
        ...
        public async Task<ActiveSpeakerObserver.ActiveSpeakerObserver> CreateActiveSpeakerObserverAsync(ActiveSpeakerObserverOptions activeSpeakerObserverOptions)
        {
            ...
            using (await _closeLock.ReadLockAsync())
            {
                ...
                var activeSpeakerObserver = new ActiveSpeakerObserver.ActiveSpeakerObserver(_loggerFactory,
        ...
        public async Task<AudioLevelObserver.AudioLevelObserver> CreateAudioLevelObserverAsync(AudioLevelObserverOptions audioLevelObserverOptions)
        {
            ...
            using (await _closeLock.ReadLockAsync())
            {
                ...
                var audioLevelObserver = new AudioLevelObserver.AudioLevelObserver(_loggerFactory,
                                    new RtpObserverInternal(_internal.RouterId, reqData.RtpObserverId),
                                    _channel,
                                    _payloadChannel,
