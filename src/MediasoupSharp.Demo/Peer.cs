...
using MediasoupSharp.Demo.Models;
...
namespace MediasoupSharp.Demo
{
    ...
    public partial class Peer
    {
        ...
        private readonly Dictionary<string, Consumer.Consumer> _consumers = new();
        ...
        public async Task<Dictionary<string, Producer.Producer>> GetProducersASync()
        {
        ...
        private readonly Dictionary<string, DataConsumer.DataConsumer> _dataConsumers = new();
        ...
        /// 创建 WebRtcTransport
        /// </summary>
        public async Task<WebRtcTransport.WebRtcTransport> CreateWebRtcTransportAsync(CreateWebRtcTransportRequest createWebRtcTransportRequest, bool isSend)
        {
            var webRtcTransportOptions = new MediasoupSharp.WebRtcTransport.WebRtcTransportOptions
            {
                ...
                await using(await _roomLock.ReadLockAsync())
                {
                    ...
                    await using(await _transportsLock.WriteLockAsync())
                    {
                        _transports[transport.TransportId] = isSend switch
                        {
                            ...
                            true when HasProducingTransport() => throw new Exception(
                                "CreateWebRtcTransportAsync() | Producing transport exists"),
                            _ => transport
                        };
                    }
        ...
        private async Task<PeerPullResult> PullInternalAsync(Peer producerPeer, IEnumerable<string> sources)
        {
            ...
            await using(await producerPeer._producersLock.ReadLockAsync())
            {
                ...
                var existsProducers = new HashSet<Producer.Producer>();
        ...
        /// 消费
        /// </summary>
        public async Task<Consumer.Consumer?> ConsumeAsync(Peer producerPeer, string producerId)
        {
            await using(await _joinedLock.ReadLockAsync())
            {
        ...
        /// 恢复消费
        /// </summary>
        public async Task<Consumer.Consumer> ResumeConsumerAsync(string consumerId)
        {
            await using(await _joinedLock.ReadLockAsync())
            {
        ...
        /// 设置消费建议 Layers
        /// </summary>
        public async Task<bool> SetConsumerPreferredLayersAsync(SetConsumerPreferredLayersRequest setConsumerPreferredLayersRequest)
        {
            await using(await _joinedLock.ReadLockAsync())
            {
                ...
                await using(await _roomLock.ReadLockAsync())
                {
                    ...
                    await using(await _consumersLock.ReadLockAsync())
                    {
                        ...
                        {
                            throw new Exception($"SetConsumerPreferredLayersAsync() | Peer:{PeerId} has no Consumer:{setConsumerPreferredLayersRequest.ConsumerId}.");
                        }
