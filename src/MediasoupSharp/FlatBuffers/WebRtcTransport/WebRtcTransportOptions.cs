...
namespace FlatBuffers.WebRtcTransport
{
    public struct WebRtcTransportOptions : IFlatbufferObject
    {
        ...
        public Listen ListenType { get { int o = __p.__offset(6); return o != 0 ? (Listen)__p.bb.Get(o + __p.bb_pos) : WebRtcTransport.Listen.NONE; } }
        ...
        public ListenIndividual ListenAsListenIndividual() { return Listen<ListenIndividual>().Value; }
        public ListenServer ListenAsListenServer() { return Listen<ListenServer>().Value; }
        ...
        public static Offset<WebRtcTransportOptions> CreateWebRtcTransportOptions(FlatBufferBuilder builder,
                                                                                  Offset<Options> @baseOffset = default(Offset<Options>),
                                                                                  Listen listen_type = WebRtcTransport.Listen.NONE,
                                                                                  int listenOffset = 0,
        ...
        public static void AddListenType(FlatBufferBuilder builder, Listen listenType) { builder.AddByte(1, (byte)listenType, 0); }
        ...
        public static Offset<WebRtcTransportOptions> EndWebRtcTransportOptions(FlatBufferBuilder builder)
        {
            int o = builder.EndTable();
            builder.Required(o, 4);  // base
            builder.Required(o, 8);  // listen
            return new Offset<WebRtcTransportOptions>(o);
        }
        ...
        public void UnPackTo(WebRtcTransportOptionsT _o)
        {
            ...
            _o.Listen = new ListenUnion();
            _o.Listen.Type = this.ListenType;
            switch(this.ListenType)
            {
                default:
                    break;
                case WebRtcTransport.Listen.ListenIndividual:
                    _o.Listen.Value = this.Listen<ListenIndividual>().HasValue ? this.Listen<ListenIndividual>().Value.UnPack() : null;
                    break;
                case WebRtcTransport.Listen.ListenServer:
                    _o.Listen.Value = this.Listen<ListenServer>().HasValue ? this.Listen<ListenServer>().Value.UnPack() : null;
                    break;
            }
        ...
        public static Offset<WebRtcTransportOptions> Pack(FlatBufferBuilder builder, WebRtcTransportOptionsT _o)
        {
            if(_o == null)
                return default(Offset<WebRtcTransportOptions>);
            var _base        = _o.Base   == null ? default(Offset<Options>) : Options.Pack(builder, _o.Base);
            var _listen_type = _o.Listen == null ? WebRtcTransport.Listen.NONE : _o.Listen.Type;
            var _listen      = _o.Listen == null ? 0 : ListenUnion.Pack(builder, _o.Listen);
