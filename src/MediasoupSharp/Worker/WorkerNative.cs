...
#pragma warning restore CS8625 // Cannot convert null literal to non-nullable reference type.
        ...
        PayloadChannel = new PayloadChannelNative(LoggerFactory.CreateLogger<PayloadChannelNative>(), threadId);
        payloadChannelPtr = GCHandle.ToIntPtr(GCHandle.Alloc(PayloadChannel, GCHandleType.Normal));
    }
    void Solve(int code)
    {
        switch (code)
    ...
    public async Task RunAsync()
    {
        ...
        Logger.LogDebug("Current process id {ProcessId}", Environment.ProcessId);
        Logger.LogDebug("Sub process id {ProcessId}", process.Id);
